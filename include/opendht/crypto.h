...
public:
    ...
    secure_array(secure_array<T,N> const&) = delete;
    ...
    secure_array<T,N>& operator=(const secure_array<T,N>& c) = delete;
...
public:
    ...
    bool checkSignature(const Blob& message, const Blob& signature) const {
        return crypto_sign_verify_detached(signature.data(), message.data(), message.size(), data_.data()) == 0;
    }

private:
    std::array<uint8_t, crypto_box_PUBLICKEYBYTES> data_;
};

class SigningSecretKey {
public:
    static std::unique_ptr<SigningSecretKey> generate() {
        auto ret = std::unique_ptr<SigningSecretKey>(new SigningSecretKey);
        crypto_sign_keypair((uint8_t*)ret->pk.data(), ret->key.data());
        return ret;
    }
    const SigningPublicKey& getPublicKey() const {
        return pk;
    }
    Blob sign(const Blob& message) {
        Blob ret(crypto_sign_BYTES);
        unsigned long long sig_len = ret.size();
        if (crypto_sign_detached(ret.data(), &sig_len, message.data(), message.size(), key.data()) != 0) {
...
class EcSecretKey {
public:
    static std::unique_ptr<EcSecretKey> generate() {
        auto ret = std::unique_ptr<EcSecretKey>(new EcSecretKey);
    ...
    SecureBlob computeSecret(const EcPublicKey& pub, bool sending) const {
        ...
        crypto_generichash_final(&h, (uint8_t*)ret.data(), ret.size());
        return ret;
    }
